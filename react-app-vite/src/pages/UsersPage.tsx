  ...
  const getAllUsers = async () => {
    ...
    if (role === "ADMIN") {
      try {
        ...
        const adminUsers = users.filter((user: UserModel) => user.role === "ADMIN");
        const nonAdminUsers = users.filter((user: UserModel) => user.role !== "ADMIN");

        // Keep ADMIN users at the top
        setUsers([...adminUsers, ...nonAdminUsers]);
      } catch (error) {
